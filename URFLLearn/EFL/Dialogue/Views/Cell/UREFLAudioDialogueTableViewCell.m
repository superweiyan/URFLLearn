...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.portView.image = [UIImage imageNamed:@"animals_left"];
        [self.contentView addSubview:self.portView];
        
        self.bubbleImageView = [[UIImageView alloc] init];
        UIImage *bubbyImage = [UIImage imageNamed:@"会话气泡_left"];
        self.bubbleImageView.image = [bubbyImage resizableImageWithCapInsets:UIEdgeInsetsMake(12, 18, 5, 8) resizingMode:UIImageResizingModeStretch];
...
//        [self.textView setContentHuggingPriority:UILayoutPriorityRequired forAxis:UILayoutConstraintAxisHorizontal];
        ...
        [self.portView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.leading.mas_equalTo(self.contentView).mas_offset(viewLeading);
...
//            make.trailing.mas_equalTo(self.textView.mas_trailing).mas_offset(-viewMarginLeading);
            ...
            make.trailing.mas_lessThanOrEqualTo(self.contentView);
        }];
